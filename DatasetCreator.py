...
def dataset_letter_and_dataset_number_creation():
    images = []
    filenames = []
    print 'Creating datasets...'
    images, filenames = get_images_for_creation('Dataset\*.jpg')
    counter = 0
    nameCounter = 10000
    file = open('dataset_letters.txt', 'w+')

    for image in images:
        plateObject = Plate(image)
        for imgChar in plateObject.final_chars_img:
...
def get_images_for_creation(path):
    imageList = []
    filenames = []
    for filename in glob.glob(path): 
        imageNext= cv2.imread(filename, cv2.IMREAD_COLOR)
        imageList.append(imageNext)
        filenames.append(filename)
    #end for
    return imageList, filenames
